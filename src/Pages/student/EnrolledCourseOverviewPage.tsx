...
export default function EnrolledCourseOverviewPage() {
    ...
    return (<motion.div initial="hidden" animate="show" variants={container} className="space-y-6">
      ...
      <motion.div variants={item}>
        <button onClick={() => navigate("/dashboard/courses/enrolled")} className={`group inline-flex items-center gap-2 text-sm font-medium transition-colors ${isDark ? "text-white/60 hover:text-white" : "text-mg-light-textSecondary hover:text-black"}`}>
      ...
      <motion.div variants={item} className={`overflow-hidden rounded-[2rem] border shadow-xl ${isDark ? "border-white/10 bg-black/25" : "border-black/10 bg-white"}`}>
        <div className="p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex-1">
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-2">
                <span className={`text-sm font-bold ${isDark ? "text-white" : "text-mg-light-text"}`}>{"Overall Progress"}</span>
          ...
          <div className="shrink-0">
            <Link to={`/dashboard/courses/${slug}/lessons/3`} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-mg-gold px-8 py-4 text-sm font-bold uppercase tracking-wider text-black shadow-[0_0_20px_rgba(212,175,55,0.3)] transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(212,175,55,0.5)] md:w-auto">
      ...
      {/* Class/Lesson View */}
      <StudentSectionCard title={"Course Syllabus"} description={"Select a lesson below to start watching."}>
        <div className="space-y-8">
          {courseData.modules.map((mod, mIdx) => (<div key={mIdx}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                return (<motion.div variants={item} key={lesson.id}>
                      <Link to={isLocked ? "#" : `/dashboard/courses/${slug}/lessons/${lesson.id}`} className={`group relative block h-full overflow-hidden rounded-2xl border p-5 transition-all duration-300 ${isLocked
      ...
      {/* Related Courses */}
      <StudentSectionCard title={"Related Courses"} description={"Expand your knowledge with these recommendations."}>
        <div className="grid gap-4 md:grid-cols-2">
          {relatedCourses.map((course) => (<motion.div variants={item} key={course.title}>
