...
export default function MilestonesPage() {
    ...
    return (<motion.div initial="hidden" animate="show" variants={container} className="space-y-5">
      <StudentSectionCard title={"Milestones"} description={"Celebrate your achievements and growth inside the platform."}>
        <motion.div variants={container} initial="hidden" animate="show" className="mb-8 grid gap-4 md:grid-cols-3">
          <motion.div variants={item} className={`rounded-2xl border p-5 ${isDark ? "border-white/10 bg-black/25" : "border-black/10 bg-white/50"}`}>
            <p className="text-sm font-medium text-mg-gold">{"Milestones Unlocked"}</p>
          ...
          <motion.div variants={item} className={`rounded-2xl border p-5 ${isDark ? "border-white/10 bg-black/25" : "border-black/10 bg-white/50"}`}>
            <p className="text-sm font-medium text-mg-gold">{"Total XP Earned"}</p>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {milestones.map((milestone) => {
            ...
            return (<motion.div variants={item} key={milestone.title} className={`relative overflow-hidden rounded-3xl border p-6 transition-all duration-300 ${milestone.unlocked
                ...
                <div className={`inline-flex rounded-2xl p-3 mb-4 transition-colors ${milestone.unlocked ? `bg-mg-gold/15 text-mg-gold` : `${isDark ? "bg-white/5 text-white/40" : "bg-black/5 text-black/40"}`}`}>
                  <Icon size={24}/>
                </div>
