...
export default function CoursesPage() {
    ...
    return (<motion.div initial="hidden" animate="show" variants={container}>
      <StudentSectionCard title={"All Courses"} description={"This page presents the full course catalog available on the platform, whether the user is already enrolled or not."}>
      <div className="grid gap-4 lg:grid-cols-2">
        {courses.map((course) => {
            const slug = course.title.toLowerCase().replace(/\s+/g, "-");
            return (<motion.div variants={item} key={course.title}>
              <Link to={`/dashboard/courses/${slug}`} className={`group relative block overflow-hidden rounded-[1.75rem] border p-5 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${isDark
              ...
              <div>
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className={`rounded-full px-3 py-1 text-xs font-bold tracking-wider ${course.price === "Free"
