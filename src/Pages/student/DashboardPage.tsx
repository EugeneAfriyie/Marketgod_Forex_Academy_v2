import { Award, BookOpen, CalendarDays, CreditCard, LibraryBig, UserCircle2, Coins } from "lucide-react";
    ...
    return (<motion.div initial="hidden" animate="show" variants={container} className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <StudentSectionCard title={"Recent Activity"} description={"The latest updates from across the platform."}>
            ...
            return (<motion.div variants={item} key={feedItem.title}>
                    <Link to={feedItem.to} className={`group flex items-start gap-4 rounded-2xl p-4 transition-all duration-300 ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}>
        ...
        <div className="flex flex-col gap-5">
          <StudentSectionCard title={"My Progress"} description={"Overview of your courses."}>
          ...
          <StudentSectionCard title={"Quick Links"} description={"Fast access to key areas."}>
            <div className="grid grid-cols-2 gap-3">
            {quickActions.map((action) => {
            const Icon = action.icon;
            return (<motion.div variants={item} key={action.title}>
                <Link to={action.to} className={`block h-full group flex flex-col items-center justify-center gap-2 rounded-2xl border p-4 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-mg-gold/50 ${isDark ? "border-white/10 bg-white/[0.03] hover:bg-white/5" : "border-black/10 bg-white/50 hover:bg-white"}`}>
                  <Icon size={20} className="text-mg-gold"/>
